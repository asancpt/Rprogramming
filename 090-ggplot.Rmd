...
```{r}
plot(acs$BMI, acs$LDLC)
grid()        # Add gridlines
# Create a linear regression model
model <- lm(formula = LDLC ~ BMI, # 순서에 조심!
            data = acs)
...
```{r}
qplot(acs$BMI, acs$LDLC)
ggplot(acs, aes(x = BMI, y = LDLC)) + geom_point()
```
...
ggplot(acs, aes(x = BMI, y = LDLC, col=Dx)) + geom_point()
