...
AMT\_IV. IV dose amount when given (ug)

AMT\_ORAL. Oral dose amount when given (ug)
...
data(dapa\_IV\_oral) Format

A data frame with 1536 rows and 11 variables

\begin{figure}

{\centering \includegraphics[width=0.75\linewidth]{images/cover} 

}

\caption{Despite what you might find at family friendly waterparks -- this is NOT how real pirate swimming lessons look.}\label{fig:unnamed-chunk-25}
\end{figure}
...
\KeywordTok{mean}\NormalTok{(aht_trial2}\OperatorTok{$}\NormalTok{AGE)}
\NormalTok{## [1] 49.72}

\CommentTok{# What was the tallest pirate?}
\KeywordTok{max}\NormalTok{(aht_trial2}\OperatorTok{$}\NormalTok{WEIGHT)}
\NormalTok{## [1] 87.05}

\CommentTok{# How many aht_trial2 are there of each sex?}
\KeywordTok{table}\NormalTok{(aht_trial2}\OperatorTok{$}\NormalTok{RACE)}
\NormalTok{## }
\NormalTok{##     Asian     Black Caucasian  Hispanic     Other }
\NormalTok{##        11        23        47         8        11}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{aggregate}\NormalTok{(}\DataTypeTok{formula =}\NormalTok{ DBP }\OperatorTok{~}\StringTok{ }\NormalTok{RACE,}
          \DataTypeTok{data =}\NormalTok{ aht_trial2,}
          \DataTypeTok{FUN =}\NormalTok{ mean)}
\NormalTok{## Error in eval(expr, envir, enclos): 객체 'DBP'를 찾을 수 없습니다}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{plot}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ aht_trial2}\OperatorTok{$}\NormalTok{DELSBP,        }\CommentTok{# X coordinates}
     \DataTypeTok{y =}\NormalTok{ aht_trial2}\OperatorTok{$}\NormalTok{DBP)        }\CommentTok{# y-coordinates}
\end{Highlighting}
\end{Shaded}

\includegraphics{Rprogramming_files/figure-latex/unnamed-chunk-32-1.pdf}
...
\includegraphics{Rprogramming_files/figure-latex/unnamed-chunk-33-1.pdf}
...
\includegraphics{Rprogramming_files/figure-latex/unnamed-chunk-34-1.pdf}

\begin{Shaded}
\begin{Highlighting}[]

\CommentTok{# Create a linear regression model}
\NormalTok{model <-}\StringTok{ }\KeywordTok{lm}\NormalTok{(}\DataTypeTok{formula =}\NormalTok{ DBP }\OperatorTok{~}\StringTok{ }\NormalTok{DELSBP, }\CommentTok{# 순서에 조심!}
            \DataTypeTok{data =}\NormalTok{ aht_trial2)}
\NormalTok{## Error in eval(expr, envir, enclos): 객체 'DBP'를 찾을 수 없습니다}

\KeywordTok{abline}\NormalTok{(model, }\DataTypeTok{col =} \StringTok{'blue'}\NormalTok{)      }\CommentTok{# Add regression to plot}
\NormalTok{## Error in abline(model, col = "blue"): 객체 'model'를 찾을 수 없습니다}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{pirateplot}\NormalTok{(}\DataTypeTok{formula =}\NormalTok{ DBP }\OperatorTok{~}\StringTok{ }\NormalTok{RACE, }
           \DataTypeTok{data =}\NormalTok{ aht_trial2,}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{pirateplot}\NormalTok{(}\DataTypeTok{formula =}\NormalTok{ SCREATININE }\OperatorTok{~}\StringTok{ }\NormalTok{GENDER,               }\CommentTok{# Plot weight as a function of sex}
           \DataTypeTok{data =}\NormalTok{ aht_trial2,                       }
...
\includegraphics{Rprogramming_files/figure-latex/unnamed-chunk-36-1.pdf}
...
the ages of aht\_trial2 who do wear a headband, and those who do not:
